...
def _map_prediction_to_label(pred):
    """
    Safely convert model output to a string label.
    Handles numpy.int64, plain int, or string inputs.
    """
    # Already a string (e.g. model returns label directly)
    if isinstance(pred, str):
        return pred.lower()
    # Numeric (numpy.int64, int, float)
    try:
        pred_int = int(pred)
...
# ── SCAN EMAIL ─────────────────────────────────────────────────────────────────
    ...
    with right:
        ...
        if analyze and email_text.strip():
            ...
            # ── DEBUG: show raw result so you can verify model output ──────────
            with st.expander("🛠 Debug — raw model output (remove before production)"):
                st.json({
                    "prediction":    str(result.get("prediction")),
                    "confidence":    result.get("confidence"),
                    "risk_level":    result.get("risk_level"),
                    "probabilities": {str(k): float(v) for k, v in result.get("probabilities", {}).items()},
                    "lime_positive": lime_result.get("positive", []) if lime_result else [],
                    "lime_negative": lime_result.get("negative", []) if lime_result else [],
                })
            ...
            risk        = result.get("risk_level", "—")
            risk_cls    = (
                "risk-high"   if "high" in str(risk).lower() else
                "risk-medium" if "med"  in str(risk).lower() else
                "risk-low"
            )
            ...
            if probs:
                # FIX: str(c) handles numpy.int64 keys safely
                ...
                fig_prob = px.bar(
                    ...
                    color_discrete_map={
                        ...
                        # Numeric fallbacks in case keys come through as "0","1","2"
                        "0": "#34D399",
                        "1": "#60A5FA",
                        "2": "#FCD34D",
                    },
                ...
                st.warning("⚠️ No probability data returned by model — check `predict_email()` return value.")
            ...
            if lime_result:
                ...
                if pos_words or neg_words:
                    ...
                    for word in pos_words:
                        pills += f'<span class="lime-pill lime-pos">+ {word}</span>'
                    for word in neg_words:
                        pills += f'<span class="lime-pill lime-neg">- {word}</span>'
                    st.markdown(f'<div class="lime-wrap">{pills}</div>', unsafe_allow_html=True)
                else:
                    st.info("LIME ran but returned no influential words for this email.")
            else:
                st.warning("⚠️ LIME explanation not returned — check `get_lime_explanation()` in phishdetect_util.py.")
...
# ── ABOUT ──────────────────────────────────────────────────────────────────────
    ...
    <div class="pd-card">
      ...
      <p style="margin-top:12px">The model uses TF-IDF vectorization and a logistic regression classifier, achieving ~89% accuracy on test data.</p>
